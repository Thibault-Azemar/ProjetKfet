...
public class CommandController {
    ...
    Iterable<Command> getDayCommands() throws Exception {
        ...
        for(Command command : allCommands) {
            ...
            if (command.getDate().getDay() == today.getDay() && command.getDate().getMonth() == today.getMonth() && command.getDate().getYear() == today.getYear()) {
    ...
    Iterable<Command> getWeekCommands() throws Exception {
        ...
        for(Command command : allCommands) {
            // TODO : changer le fonctionnement car le code est déprécié
            if (command.getDate().getDay() >= today.getDay() - 7 && command.getDate().getMonth() == today.getMonth() && command.getDate().getYear() == today.getYear()) {
...
//        On ajoute les autres champs à la commande
        ...
        // TODO : Boolean.parseBoolean(isPaid). Utile lorsque cela sera implémenté en front
        o.setIsPaid(true);
    ...
    String UpdateStateProductCommand(@RequestParam("idCommand") String id, @RequestParam("idProductInList") String idproduct, @RequestParam("state") String state) throws Exception {
        ...
        if (o.isPresent())
        {
            ...
            if (isProductInList)
            {
                // Si tous les champs state sont à "Terminé", on passe la commande à "Terminé". Sinon, on passe à "En cours". Si tous les champs sont
